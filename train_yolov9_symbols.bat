...
set "DATA=%~dp0articulation_experiments\outputs\yolo_dataset_extended\dataset.yaml"
set "RUN_DIR=%~dp0articulation_experiments\outputs\runs\yolov9_symbols_v1"
...
"%PYTHON%" articulation_experiments\train\yolov9_compat_launcher.py ^
  ...
  --project "%~dp0articulation_experiments\outputs\runs" ^
  --name yolov9_symbols_v1 --seed 20260723
if errorlevel 1 goto :failed

echo YOLOv9 40-class symbol training completed.
exit /b 0
