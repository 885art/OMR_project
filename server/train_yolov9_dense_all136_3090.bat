...
set "DATASET_ROOT=C:\OMR_work\experiments\datasets\deepscores_dense_all136_1024"
...
set "RUN_NAME=yolov9_e_deepscores_dense_all136_30ep_b4_3090"
set "EPOCHS=30"
set "BATCH_SIZE=4"
...
if not exist "%DATASET_ROOT%\dataset.yaml" (echo [ERROR] Run prepare_deepscores_dense_all136_3090.ps1 -Mode Full first. & exit /b 1)
if not exist "%DATASET_ROOT%\validation_report.json" (echo [ERROR] Missing validation report. & exit /b 1)
if not exist "%WEIGHTS%" (echo [ERROR] Missing initial checkpoint: %WEIGHTS% & exit /b 1)
...
if /I "%~1"=="preflight" (echo Dense all136 preflight passed. & exit /b 0)
if /I "%~1"=="resume" (
...
echo Starting Dense all136 training: epochs=%EPOCHS% batch=%BATCH_SIZE% imgsz=1024 device=0
