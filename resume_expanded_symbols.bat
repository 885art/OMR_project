...
set "CHECKPOINT=%~dp0articulation_experiments\outputs\runs\expanded_symbols_v1\weights\last.pt"
if not exist "%PYTHON%" (
...
if not exist "%CHECKPOINT%" (
  ...
  echo Run train_expanded_symbols.bat first.
  pause
  exit /b 1
)

echo Resuming expanded-symbol training from last.pt...
